...
// infer 推导
type FirstEl<T extends unknown[]> = T extends [infer First, ...infer Other] ? First : never
...
type getArrExcludeFirst<T extends unknown[]> = T extends [unknown, ...infer ExcludeType] ? ExcludeType : never
type getArrExcludeLast<T extends unknown[]> = T extends [...infer ExcludeType, unknown] ? ExcludeType : never
...
// 模式匹配 针对字符串
type startsWith<T extends string, Prefix extends string> = Prefix extends '' ? false : T extends `${Prefix}${string}` ? true : false

type ReplaceStr<T extends string, From extends string, To extends string> = T extends `${infer Prefix}${From}${infer affix}` ? `${Prefix}${To}${affix}` : never
